...
async def handle_animation_request():
    ...
    try:
        ...
        required_keys = ["parameter", "start_date", "end_date", "fps", "frames_per_transition"]
        ...
        # 1. Fetch data
        df = await fetch_data_for_animation(parameter, start_date, end_date)
        if df.empty:
            return jsonify({"error": "No data available for the selected parameter and date range."}), 404

        # 2. Generate video
        video_bytes = generate_spatiotemporal_video(df, fps, frames_per_transition, cmap)
